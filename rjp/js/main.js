...
window.onload = function() {
    ...
    game_.preload('./img/chara2.png', './img/start.png', './img/gameover.png','./img/bgg7.png', './img/bgg8.png', './img/hurdle.png', './img/minihurdle.png', './img/bird.png'); // ゲームに使う素材を、あらかじめ読み込む
    game_.onload = function() { // ゲームの準備が整ったらメインの処理を実行
      ...
      */
      var createGameScene = function() {
        ...
        // ミニハードルの設定
        var miniHurdleA = new Sprite(45, 35);          // スプライトを作る
        miniHurdleA.image = game_.assets['./img/minihurdle.png']; // 画像を設定
        miniHurdleA.x = -miniHurdleA.width;                // 横位置調整 画面外に隠しておく
        miniHurdleA.y = (GROUND_LINE - miniHurdleA.height) * 1.02;  // 縦位置調整 ミニハードル下端を地面の高さと合わせる
        scene.addChild(miniHurdleA);                   // シーンに追加

        // ミニハードルの設定２
        var miniHurdleB = new Sprite(45, 35);          // スプライトを作る
        miniHurdleB.image = game_.assets['./img/minihurdle.png']; // 画像を設定
        miniHurdleB.x = -miniHurdleB.width;                // 横位置調整 画面外に隠しておく
        miniHurdleB.y = (GROUND_LINE - miniHurdleB.height) * 1.02;  // 縦位置調整 ミニハードル下端を地面の高さと合わせる
        scene.addChild(miniHurdleB);                   // シーンに追加

        // ミニハードルの設定３
        var miniHurdleC = new Sprite(45, 35);          // スプライトを作る
        miniHurdleC.image = game_.assets['./img/minihurdle.png']; // 画像を設定
        miniHurdleC.x = -miniHurdleC.width;                // 横位置調整 画面外に隠しておく
        miniHurdleC.y = (GROUND_LINE - miniHurdleC.height) * 1.02;  // 縦位置調整 ミニハードル下端を地面の高さと合わせる
        scene.addChild(miniHurdleC);                   // シーンに追加
        ...
        // くれの設定
        var Kure = new Sprite(32, 32);             // スプライトを作る
        Kure.image = game_.assets['./img/chara2.png']; // 画像を設定
        Kure.x = 80;                               // 横位置調整 画面左側に配置
        Kure.y = GROUND_LINE - Kure.height;        // 縦位置調整 くれの下端を地面の高さに合わせる
        scene.addChild(Kure);                      // シーンに追加
        // くれの当たり判定用スプライトの設定
        var Kure_hit = new Sprite(1, 1);           // スプライトを作る（幅1, 高さ1）
        // Kure_hit.image =                        // 画像は設定しない（透明）
        Kure_hit.x = Kure.x + Kure.width / 2;      // 横位置調整 くれの左右中央に配置
        Kure_hit.y = Kure.y + Kure.height / 2;     // 縦位置調整くれの上下中央に配置
        scene.addChild(Kure_hit);                  // シーンに追加

        // くれがやられた関数
        var KureDead = function() {
          Kure.frame = 3;                       // くれを涙目にする
        ...
        scene.addEventListener(Event.TOUCH_START, function(e){
          // くれをジャンプさせる
          Kure.tl.moveBy(0, -80, 9, enchant.Easing.CUBIC_EASEOUT) // 12フレームかけて現在の位置から上に120px移動
      ...
      */
      var createGameoverScene = function(resultScore) {
        ...
        gameover.x = 0;                                      // 横位置調整
        gameover.y = 0;                                     // 縦位置調整
        gameover.font = '20px sans-serif';
        scene.addChild(gameover);                             // シーンに追加

        //ランキング見出し
        var head = new Label('rank score  ');
        head.textAlign ='center';
        head.color = '#ffffff';
        head.x = 0;
        head.y = 70;
        head.font = '20px sans-serif';
        scene.addChild(head);

        //ランキングラベル設定（ダミー）
        var rank = 10000;
        var rank1 = new Label('１.　'+rank);                   // スプライトを作る
        rank1.textAlign = 'center';
        rank1.color = '#ffffff';
        rank1.x = 0;                                      // 横位置調整
        rank1.y = 100;                                     // 縦位置調整
        rank1.font = '20px sans-serif';
        scene.addChild(rank1);                             // シーンに追加

        //ランキングラベル設定（ダミー２）
        var rank = 10000;
        var rank2 = new Label('２.　'+rank);                   // スプライトを作る
        rank2.textAlign = 'center';
        rank2.color = '#ffffff';
        rank2.x = 0;                                      // 横位置調整
        rank2.y = 120;                                     // 縦位置調整
        rank2.font = '20px sans-serif';
        scene.addChild(rank2);                             // シーンに追加

        //ランキングラベル設定（ダミー３）
        var rank = 10000;
        var rank3 = new Label('３.　'+rank);                   // スプライトを作る
        rank3.textAlign = 'center';
        rank3.color = '#ffffff';
        rank3.x = 0;                                      // 横位置調整
        rank3.y = 140;                                     // 縦位置調整
        rank3.font = '20px sans-serif';
        scene.addChild(rank3);                             // シーンに追加

        //ランキングラベル設定（ダミー４）
        var rank = 10000;
        var rank4 = new Label('４.　'+rank);                   // スプライトを作る
        rank4.textAlign = 'center';
        rank4.color = '#ffffff';
        rank4.x = 0;                                      // 横位置調整
        rank4.y = 160;                                     // 縦位置調整
        rank4.font = '20px sans-serif';
        scene.addChild(rank4);                             // シーンに追加

        //ランキングラベル設定（ダミー５）
        var rank = 10000;
        var rank5 = new Label('５.　'+rank);                   // スプライトを作る
        rank5.textAlign = 'center';
        rank5.color = '#ffffff';
        rank5.x = 0;                                      // 横位置調整
        rank5.y = 180;                                     // 縦位置調整
        rank5.font = '20px sans-serif';
        scene.addChild(rank5);                             // シーンに追加
        ...
        label.x = 0;                                               // 横位置調整
        label.y = 220;                                              // 縦位置調整
        label.font = '20px sans-serif';                            // 40pxのゴシック体にする
        ...
        //タイトルに戻るラベル(ボタン)設定
        var exit = new Label("タイトルに戻る");
        exit.color = '#ffffff';
        exit.x = 180;
        exit.y = 300;
        exit.font = '20px sans-self';
        scene.addChild(exit);
